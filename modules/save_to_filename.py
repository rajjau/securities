...
### MAIN ###
############
def main(dir_data_saved, name, symbols, extension, random_state = False, timestamp = False):
    ...
    name = name.replace(' ', '_')
    # Build the filename using symbols.
    filename = Path(f"{name}_{symbols}_{timestamp}.{extension}")
